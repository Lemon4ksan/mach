...
func (u *URI) parse(host, uri []byte, isTLS bool) error {
	...
	if isTLS {
		u.SetSchemeBytes(bytesutil.StrHTTPS)
	}
...
func ishex(c byte) bool {
	return bytesutil.Hex2intTable[c] < 16
}

func unhex(c byte) byte {
	return bytesutil.Hex2intTable[c] & 15
}
...
func normalizePath(dst, src []byte) []byte {
	...
	// remove /./ parts
	for {
		n := bytes.Index(b, bytesutil.StrSlashDotSlash)
		if n < 0 {
			break
		}
	...
	// remove /foo/../ parts
	for {
		n := bytes.Index(b, bytesutil.StrSlashDotDotSlash)
		if n < 0 {
			break
		}
	...
	// remove trailing /foo/..
	n := bytes.LastIndex(b, bytesutil.StrSlashDotDot)
	if n >= 0 && n+len(bytesutil.StrSlashDotDot) == len(b) {
	...
	if filepath.Separator == '\\' {
		// remove \.\ parts
		for {
			n := bytes.Index(b, bytesutil.StrBackSlashDotBackSlash)
			if n < 0 {
				break
			}
		...
		// remove /foo/..\ parts
		for {
			n := bytes.Index(b, bytesutil.StrSlashDotDotBackSlash)
			if n < 0 {
				break
			}
		...
		// remove /foo\..\ parts
		for {
			n := bytes.Index(b, bytesutil.StrBackSlashDotDotBackSlash)
			if n < 0 {
				break
			}
		...
		// remove trailing \foo\..
		n := bytes.LastIndex(b, bytesutil.StrBackSlashDotDot)
		if n >= 0 && n+len(bytesutil.StrSlashDotDot) == len(b) {
			nn := bytes.LastIndexByte(b[:n], '/')
			if nn < 0 {
				return append(dst[:0], bytesutil.StrSlash...)
...
func (u *URI) Update(newURI string) {
	u.UpdateBytes(bytesutil.S2B(newURI))
}
...
func (u *URI) updateBytes(newURI, buf []byte) []byte {
	...
	n := bytes.Index(newURI, bytesutil.StrSlashSlash)
...
func (u *URI) appendSchemeHost(dst []byte) []byte {
	...
	dst = append(dst, bytesutil.StrColonSlashSlash...)
