...
func clientPostURL(dst []byte, url string, postArgs *Args, c clientDoer) (statusCode int, body []byte, err error) {
	...
	req.Header.SetContentTypeBytes(bytesutil.StrPostArgsContentType)
	if postArgs != nil {
...
var (
	...
	ErrMissingLocation = errors.New("mach: missing location header for http redirect")
...
func doRequestFollowRedirects(
	...
	for {
		...
		location := resp.Header.peek(bytesutil.StrLocation)
		if len(location) == 0 {
...
func hostnameFromURLString(url string) []byte {
	_, host, _ := splitHostURI(nil, bytesutil.S2B(url))
...
func (c *HostClient) DoPipelineTimeout(reqs []*Request, resps []*Response, timeout time.Duration) error {
	...
	if userAgentOld == "" && !c.NoDefaultUserAgentHeader {
		userAgentOld = bytesutil.DefaultUserAgent
	}
...
func (c *HostClient) doNonNilReqResp(req *Request, resp *Response) (bool, error) {
	...
	if len(userAgentOld) == 0 {
		...
		if userAgent == "" && !c.NoDefaultUserAgentHeader {
			userAgent = bytesutil.DefaultUserAgent
		}
		if userAgent != "" {
...
var (
	...
	// see this error.
	ErrNoFreeConns = errors.New("mach: no free connections available to host")
	...
	// to broken server.
	ErrConnectionClosed = errors.New("mach: the server closed connection before returning the first response byte. " +
	...
	ErrStaleConnection = errors.New("mach: stale connection")
	...
	ErrConnPoolStrategyNotImpl = errors.New("mach: connection pool strategy is not implement")
)
...
var ErrTLSHandshakeTimeout = errors.New("mach: tls handshake timed out")
...
func (w *wantConn) tryDeliver(conn *clientConn, err error) bool {
	...
	w.err = err
	if w.conn == nil && w.err == nil {
		panic("mach: internal error: misuse of tryDeliver")
	}
	close(w.ready)
	return true
}
...
var ErrPipelineOverflow = errors.New("mach: pipelined requests' queue has been overflowed. " +
