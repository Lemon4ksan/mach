...
func TestH2_FrameHeader_Adversarial(t *testing.T) {
	...
	fr := h2.AcquireFrameHeader()
	defer h2.ReleaseFrameHeader(fr)
	...
	for _, tc := range testCases {
		...
		var buf [9]byte
		h2.PackFrameHeader(buf[:], tc.length, tc.fType, tc.flags, tc.streamID)
		length, kind, flags, stream := h2.UnpackFrameHeader(buf[:])
